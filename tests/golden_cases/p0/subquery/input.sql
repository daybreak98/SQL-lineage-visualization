        ...
        FROM
            (
                ...
                WHERE dt between '2025-05-13' AND '2026-05-13'
                  AND sale_channel='Q2Q'
                  AND is_international='1'
                  AND order_status <> 'DELETE'
                  AND (((balance_type='PROXY' OR is_guarantee=1) AND pay_status NOT IN ('PAY','PAY_FAILED')) OR (balance_type='CASH' AND is_guarantee='0'))
                  AND checkin_date between '2026-03-13' and '2026-05-13'
            )a1
                left join
            (
