...
import { diagnostics, mappings, sourceLocations } from '../data/mockLineage';
import { diagnosticsOf, entityName, entityOf, buildPathContext } from '../data/selectors';
...
export function DetailPanel({ state, setState }: Props) {
  ...
  if (state.detailMode !== 'expanded') {
    ...
    const line2 = first ? `source: ${entityName(first.source)} → target: ${entityName(first.target)} · ${first.relation}` : `Upstream structure · ${diagnosticsOf(state.selectedEntity).length} diagnostics`;
    ...
    const ds = diagnosticsOf(state.selectedEntity);
    ...
    <div className={cx('detail', state.detailMode === 'collapsed' && 'collapsed', state.detailMode === 'expanded' && 'expanded')}>
      <div className="detail-head">
        ...
        <div className="flex items-center gap-1">
          <button className="btn h-[26px] text-[11px] bg-blue-50 border-blue-200 text-blue-700" onClick={() => setTab('source')}>Locate SQL</button>
